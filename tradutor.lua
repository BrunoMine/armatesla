...
-- Tradução intllib
armatesla.intllib = {}
armatesla.intllib.S, armatesla.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
armatesla.S = armatesla.intllib.S
armatesla.NS = armatesla.intllib.NS
...
-- Gera arquivos de tradução armatesla.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por armatesla apartir de "..file.."\n# textdomain: armatesla\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/armatesla."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("armatesla")
else
	s = armatesla.intllib.S
end

armatesla.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[armatesla] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, armatesla.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return armatesla.s(unpack(r))
...
armatesla.Sfake = function(s) return s end
